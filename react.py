...
def custom_collate(batch):
    ...
    speaker_audio2text_clip = [item[9] for item in batch if len(item[9]) > 0]
    speaker_lip_clip = [item[10] for item in batch if len(item[10]) > 0]
...
class ReactionDataset(data.Dataset):
    def __init__(self,
        ...
        self.load_audio2text = load_audio2text
        ...
        self._audio2text = os.path.join(dataset_dir, 'audio2embeddings')
        self._lip = os.path.join(dataset_dir, 'lip-features')
    ...
    def __getitem__(self, index):
        ...
        if self.load_3dmm_s:
            ...
            speaker_params_clip = self._transform_3dmm(params)[0] 
        speaker_audio_clip = torch.zeros(size=(0,))
        if self.load_audio:
            if self.audio_feature_type == 'wav2vec':
                audio_path = os.fspath(Path(self._audio_dir) / speaker_path.with_suffix('.npy'))
                speaker_audio_clip = np.load(audio_path)  
                T1, D = speaker_audio_clip.shape

            elif self.audio_feature_type == 'mfcc':
                audio_path = os.fspath(Path(self._audio_dir) / speaker_path.with_suffix('.wav'))
                speaker_audio_clip = extract_audio_features(audio_path, self._fps, total_length)  
            else:
        ...
        speaker_audio2text_clip = torch.zeros(size=(0,))
        if self.load_audio2text:
            speaker_audio2text_path = os.fspath(Path(self._audio2text) / speaker_path.with_suffix('.npy'))
            speaker_audio2text_clip = np.load(speaker_audio2text_path)  
            T2 = speaker_audio2text_clip.shape[0]

            if T1 <= T2:
                speaker_audio2text_clip = speaker_audio2text_clip[:T1]
            else:
                repeat_factor = int(np.ceil(T1 / T2))
                speaker_audio2text_clip = np.tile(speaker_audio2text_clip, (repeat_factor, 1))[:T1]

            speaker_audio2text_clip = torch.from_numpy(speaker_audio2text_clip)[cp:cp + self._clip_length]
        
        speaker_lip_clip = torch.zeros(size=(0,))
        if self.load_lip:
            speaker_lip_path = os.fspath(Path(self._lip) / speaker_path.with_suffix('.npy'))
            speaker_lip_clip = np.load(speaker_lip_path)  
            T2 = speaker_lip_clip.shape[0]

            if T1 <= T2:
                speaker_lip_clip = speaker_lip_clip[:T1]
            else:
                repeat_factor = int(np.ceil(T1 / T2))
                speaker_lip_clip = np.tile(speaker_lip_clip, (repeat_factor, 1))[:T1]

            speaker_lip_clip = torch.from_numpy(speaker_lip_clip)[cp:cp + self._clip_length]
        ...
        if total_length < self._clip_length:
            ...
            speaker_audio2text_clip = torch.cat((speaker_audio2text_clip,
                                            torch.zeros(size=(self._clip_length - total_length,
                                                              speaker_audio2text_clip.shape[-1]))), dim=0) \
                if self.load_audio2text else speaker_audio2text_clip
            speaker_lip_clip = torch.cat((speaker_lip_clip,
                                            torch.zeros(size=(self._clip_length - total_length,
                                                              speaker_lip_clip.shape[-1]))), dim=0) \
                if self.load_lip else speaker_lip_clip
